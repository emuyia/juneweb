...
def upgrade():
    ...
    conn.execute(
        sa.text('UPDATE "user" SET role_id = :user_role_id WHERE role_id IS NULL').bindparams(user_role_id=user_role_id)
    )

    # Update the 'user' table, setting 'role_id' to the 'Admin' role ID for users previously marked as admins
    conn.execute(
        sa.text('UPDATE "user" SET role_id = :admin_role_id WHERE is_admin = True').bindparams(
...
def downgrade():
    ...
    conn.execute(
        sa.text('UPDATE "user" SET is_admin = True WHERE role_id = :admin_role_id').bindparams(
            admin_role_id=admin_role_id)
    )
